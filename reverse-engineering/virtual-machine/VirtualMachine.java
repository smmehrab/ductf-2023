...
public class VirtualMachine {
    private static final String[] owo = { "so you have found the flag", "Don't try me baba" };
    ...
    private VirtualMachine(int[] ins) {
    ...
    private int pop() {
        return this.stackArr[--this.sp];
    }

    private void push(int n) {
        this.stackArr[this.sp++] = n;
    }

    private int run(int... params) {
        if (params != null)
            for (int i = 0; i < params.length; i++)
                this.regs[i] = params[i];
        while (this.isRun) {
            int opc = readByte();
            int opn = readByte();
            switch (opc) {
    ...
    public static void main(String... args) {
        ...
        char[] flagFormat = { 'd', 'u', 'C', 'T', 'F' };

        if (args.length != 1 || args[0].toCharArray().length != 31) {
            System.out.println(getOutput(1));
            System.exit(0);
        }
        VirtualMachine java = new VirtualMachine(new int[] { 0xb, 0x0, 0x6, 0x0, 0x1, 0x64, 0x5, 0x14, 0xb, 0x1, 0x6,
